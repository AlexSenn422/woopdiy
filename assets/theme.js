  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _onSelectChange: function() {
      ...
      throttle = setTimeout(function(){
        var variant_selector = document.getElementById('ProductSelect-product-template');
        if(variant_selector) {
            var image = variant_selector.options[variant_selector.selectedIndex].getAttribute('data-image');
            if(image) {
              var _i = document.querySelectorAll('#product-template-pager :not(.cycle-sentinel) img[data-id="'+image+'"]');
              if(_i.length){
                var img = _i[_i.length - 1];
                if(img) {
                  autoclick = true;
                  img.click(); 
                  autoclick = false;
        ...
        BOLD.common.eventEmitter.emit('BOLD_COMMON_variant_changed', variant);
      }
    },
...
$(function(){
  var topofDiv = $("#site-header").offset().top; //gets offset of header
  var height = $("#site-header").outerHeight(); //gets height of header
